...
import { Shield, Moon, PhoneCall, AlertTriangle, Compass, MessageSquare, ShieldCheck } from 'lucide-react';
    ...
    <header className="w-full bg-[#090d18]/90 backdrop-blur-xl border-b border-white/10 sticky top-0 z-[1100] px-4 lg:px-8 py-3.5 mb-6">
      <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 cursor-pointer" onClick={() => setActiveTab('planner')}>
          <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-cyan-500 via-emerald-500 to-indigo-500 p-0.5 shadow-lg shadow-cyan-500/25">
          ...
          <div>
            ...
            <p className="text-[11px] font-medium text-slate-400 hidden sm:block">
              Smart Night-Time Route Planning Engine
            </p>
          </div>
        </div>

        {/* Tab Navigation Links */}
        <nav className="flex items-center gap-1 bg-[#050811] p-1 rounded-2xl border border-white/5">
          {NAV_TABS.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all duration-300 flex items-center gap-2 ${
                  isActive
                    ? 'bg-gradient-to-r from-cyan-500/20 to-emerald-500/20 text-cyan-300 border border-cyan-400/30 shadow-md shadow-cyan-500/10'
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-slate-200 text-xs font-bold flex items-center gap-1.5 transition-all"
          >
          ...
          <button
            onClick={onOpenSOS}
            className="px-4 py-2 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white text-xs font-extrabold flex items-center gap-1.5 shadow-lg shadow-red-600/30 transition-all active:scale-95"
          >
